...
module.exports = gulp.task('styles', function () {
  <% if (includeSass) { %>return sass(config.paths.src.styles, {'sourcemap=none': true } ).on('error', handleError)<% } else { %> return gulp.src(config.paths.src.styles) <% } %><% if (includeLess) { %>
